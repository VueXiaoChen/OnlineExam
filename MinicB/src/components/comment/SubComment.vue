<template>
    <div class="sub-reply-container">
      <div class="sub-reply-list">
        ...
        <div 
          ...
          v-for="subComment in currentPageComments" 
          :key="subComment.id"
          @mouseover="activeCommentId = subComment.id" 
          @mouseout="activeCommentId = null"
        >
          <!-- 用户信息 -->
          <div class="sub-user-info">
                ...
                <template #reference>
                  <a :href="`/space/${subComment.user.uid}`" target="_blank" class="sub-reply-avatar">
                    <VAvatar 
                      ...
                      :img="subComment.user.avatar_url" 
                      :auth="subComment.user.auth" 
                    />
                  </a>
                </template>
                <template #content>
                  <UserCard :user="subComment.user" />
                </template>
              </VPopover>
            </div>
            ...
            <VPopover pop-style="z-index: 2000; cursor: default; padding-top: 20px; left: 0; transform: translate3d(0,0,0);">
              <template #reference>
                <a 
                  :href="`/space/${subComment.user.uid}`" 
                  target="_blank" 
                  class="sub-user-name"
                  :class="{ 'vip-name': subComment.user.vip !== 0 }"
                >
            ...
            <!-- 等级 -->
            <VLevel class="level" :level="handleLevel(subComment.user.exp)" :size="12" />
            
            <!-- UP主标识 -->
            <svg 
              v-if="subComment.user.uid === upUid" 
              width="24" 
              height="24" 
              viewBox="0 0 24 24" 
              fill="none" 
          ...
          <span class="reply-content-container sub-reply-content">
            <span class="reply-content">
              <!-- 如果是回复别人的评论 -->
              <span v-if="rootId !== subComment.parentId">
                回复
                <VPopover style="display: inline-block;" pop-style="z-index: 2000; cursor: default; padding-top: 20px; left: 0; transform: translate3d(0,0,0);">
                  <template #reference>
                    <a 
                      class="jump-link" 
                      :href="`/space/${subComment.toUser?.uid}`" 
                      target="_blank"
                    >
                      @{{ subComment.toUser?.nickname || '用户' }}
                    </a>
                  </template>
                  <template #content>
                    <UserCard :user="subComment.toUser" />
                  </template>
                </VPopover>
                :
              </span>
          ...
          <div class="sub-reply-info">
            ...
            <span class="sub-reply-like">
              <i 
                class="svg-icon sub-like-icon" 
                style="width: 16px; height: 16px;"
                @click="likeOrDislike(subComment.id, true, !commentStore.isLiked(subComment.id))"
              >
                <svg 
                  v-if="!commentStore.isLiked(subComment.id)"
                  t="1710593669221" 
                  class="icon" 
            ...
            <span class="sub-reply-dislike">
              <i 
                class="svg-icon sub-dislike-icon" 
                style="width: 16px; height: 16px;"
                @click="likeOrDislike(subComment.id, false, !commentStore.isDisliked(subComment.id))"
              >
                <svg 
                  v-if="!commentStore.isDisliked(subComment.id)"
                  t="1710593747978" 
                  class="icon" 
                  viewBox="0 0 1024 1024" 
                  version="1.1"
            ...
            <div 
              ...
              :style="{ opacity: activeCommentId === subComment.id ? 1 : 0 }"
            >
        ...
        <!-- 查看更多评论 -->
        <div class="view-more" v-if="count > 3">
          <!-- 默认显示 -->
          ...
          <div class="view-more-pagination" v-else>
            ...
            <span 
              ...
              :class="{ 'current-page': currPage === index }"
  ...
  import { emojiText, handleNum, handleLevel, handleDateTime3 } from '../../utils/utils.js'
...
import axios from 'axios'
  ...
  const showOption:any = ref(false)
  const showMoreComment:any = ref(false)
  ...
  const currentPageComments = computed(() => {
    const start = (currPage.value - 1) * 10
    const end = currPage.value * 10
    return props.replies.slice(start, end)
  })
  ...
  const likeOrDislike = async (id, isLike, isSet) => {
    ...
    // 检查评论是否存在
    const commentIndex = props.replies.findIndex(comment => comment.id === id)
    if (commentIndex === -1) {
    ...
    try {
      ...
      if (res) {
        ...
        if (isLike && isSet) {
          ...
          // 更新点赞数
          const comment:any = props.replies[commentIndex]
          comment.love += 1
  ...
  const handleTime = (time) => handleDateTime3(time)
